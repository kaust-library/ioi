		...
		if( isset($_POST['import'])){
			...
			if(is_uploaded_file($_FILES['csvfile']['tmp_name']))
			{
				...
				if($accepted !== FALSE)
				{
					...
					$acceptedFields = array_map('trim', array_map('strtolower', $acceptedUploads[$accepted]['fields']));

					# the file should have the accepted number of columns and column names
					if(count($acceptedFields) == count($firstRow))
					{
						...
						while (($row = fgetcsv($file)) !== FALSE)
						{
							...
							$chunks = array_combine($firstRow, $row);

							# send the data to processing function
							$result = call_user_func('processLocal'.(ucfirst($selectedOption)).'Record', $chunks);
