...
**** This file contain the login method.
...
//-----------------------------------------------------------------------------------------------------------
	...
	function error_connection($username='')
	{
		...
		include_once 'snippets/html/loginForm.php';
	...
	else
	{
		...
		*/
		...
		foreach ($justthese as $attribute)
		{
		  // the server returns lowercase attribute names
		...
		$admins  = getValues($ioi, "SELECT `email`  FROM `users` where `admin`  = 1", array('email'), 'arrayOfValues');

		if(in_array($_SESSION[LDAP_EMAIL_ATTRIBUTE], $admins)){
